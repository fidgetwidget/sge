...
import sge.collision.shapes.*;
import sge.collision.ray.*;
...
class SAT2D {
  ...
  public static function testCircleVsPolygon( circle:Circle, polygon:Polygon, ?into:ShapeCollision, flip:Bool=false ) : ShapeCollision {
    ...
    into = into == null ? ShapeCollisionPool.instance.get() : into.reset();
  ...
  public static function testPolygonVsPolygon( polygon1:Polygon, polygon2:Polygon, ?into:ShapeCollision, flip:Bool=false ) : ShapeCollision {
    ...
    result.other = ShapeCollisionPool.instance.get();
  ...
  public static inline function vec_normalize( length :Float, component :Float ) return length == 0 ? 0 : component /= length;
