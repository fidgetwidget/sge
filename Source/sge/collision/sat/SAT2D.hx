...
import sge.collision.sat.shapes.*;
import sge.collision.sat.ray.*;
...
// 
// TODO: optimize for lower memory usage (recylced objects {Vector, ShapeCollision, etc})
// 
class SAT2D
{

  // Internal api - test a circle against a polygon
  public static function testCircleVsPolygon( circle:Circle, polygon:Polygon, ?collision :ShapeCollision, flip :Bool = false ) : ShapeCollision 
  {

    var collisionData = collision == null ? new ShapeCollision() : collision.reset();
  ...
  // test a circle against a circle
  public static function testCircleVsCircle( circle1 :Circle, circle2 :Circle, ?collision :ShapeCollision ) : ShapeCollision 
  {
  ...
  static var result1 :ShapeCollision = new ShapeCollision();
  static var result2 :ShapeCollision = new ShapeCollision();

  // test a polygon against another polygon 
  public static function testPolygonVsPolygon( polygon1 :Polygon, polygon2 :Polygon, ?collision :ShapeCollision, flip :Bool = false ) : ShapeCollision 
  {
    
    if ( checkPolygons(polygon1, polygon2, result1, flip) == null )
      return null;

    if ( checkPolygons(polygon2, polygon1, result2, !flip) == null )
      return null;

    var result = null, other = null;
    if(Math.abs(result1.overlap) < Math.abs(result2.overlap)) {
        result = result1;
        other = result2;
    } else {
        result = result2;
    ...
    collision.copy_from(result);
    collision.other = new ShapeCollision();
    collision.other.copy_from(other);
  ...
  public static function bresenhamLine( start :Vector, end :Vector ) : Array<Vector> 
  {
    ...
    var startXi :Int = Std.int(start.x);
    var endXi   :Int = Std.int(end.x);
    for(x in startXi...endXi) { //for each point
  ...
  // implementation details for testPolygonVsPolygon
  private static function checkPolygons( polygon1 :Polygon, polygon2 :Polygon, ?collision :ShapeCollision, flip :Bool = false ) : ShapeCollision 
  {
  ...
  // swap x and y of a vector, returning a new vector. :todo: this is silly
  private static inline function swap( v :Vector ) : Vector return new Vector(v.y, v.x);
